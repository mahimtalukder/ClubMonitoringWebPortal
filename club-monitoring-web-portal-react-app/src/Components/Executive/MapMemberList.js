...
const MapMemberList = (props) => {
    const navigate = useNavigate();
    const handleClickBlockUnblock = (event, { user_id,  status}) => {
        //Write your code here
        var url = "executive/update/status/member/" + user_id + "/" + status;
        console.log(url);
        AxiosConfig.get(url)
            .then(resp => {
    ...
    return (
        <tr>
            ...
            <td>
                <Link to={"/executive/update/member/"+props.user_id } className="btn btn-primary me-1 mb-1">View</Link>
